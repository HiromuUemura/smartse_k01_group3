...
### フェーズ B — 入力UI（issue #4）
- [x] B-1 アップロード画面（`app/extract-ui.tsx`：APIキー入力 / ファイル選択 / 抽出実行 / 結果表示）
- [x] B-2 抽出結果の確認＆編集フォーム（各項目を画面で修正→登録）

> ⚠️ グループでの確認・決定が必要な事項は [`docs/REVIEW_NEEDED.md`](./REVIEW_NEEDED.md) に集約（コード中は `要グループ確認` コメント）。

### フェーズ C — 不足情報フロー（issue #5, #6）
- [x] C-1 `missingFields` を画面でハイライトし補完を促す（「要確認」表示＋バナー）
- [ ] C-2 （任意）不足分の再プロンプト（Agent化の検討対象＝方針決定後）

### フェーズ D — 本番登録
- [x] D-1 `create-event` を「確定内容の登録」へ改修（issue #8）
- [x] D-2 対象(parent/family)に応じた `attendees` 自動付与（issue #10, #11）
- [x] D-3 `reminders.overrides` で前日/当日通知（issue #12, #13）

### フェーズ E — 仕上げ
- [~] E-1 エラー表示UI / ローディング / 認証ガード（主要画面は対応済み）
- [~] E-2 README更新済み。テスト整備（Vitest 等）は未
- [~] E-3 `/api/auth/google` の prerender 回避（`force-dynamic`）。デプロイ先のリダイレクトURIは未
